...
class LoaiKhachHangImport implements ToCollection
{
  ...
  protected $muc_import = 'Loại khách hàng';
  protected $model_class = LoaiKhachHang::class;
  protected $createRequest = CreateLoaiKhachHangRequest::class;
  ...
  {
    // Lấy rules từ CreateLoaiKhachHangRequest
    ...
    foreach ($data as $index => $item) {
      try {
        ...
        $rowData = [
          'ten_loai_khach_hang' => $item[0],
          'nguong_doanh_thu' => $item[1],
          'trang_thai' => $item[2],
        ];
  ...
  protected function createRecords()
  {
    foreach ($this->validated_data as $valid_item) {
      try {
        $loaiKhachHang = $this->model_class::create($valid_item['rowData']);
        $this->thanh_cong++;

        $this->ket_qua_import[] = [
          'dong' => $valid_item['index'] + 2, // +2 vì index từ 0 và đã bỏ qua header
  ...
  protected function addFailedResult($index, $item, $message, $errors)
  {
    $this->that_bai++;
    $this->ket_qua_import[] = [
      'dong' => $index + 2, // +2 vì index từ 0 và đã bỏ qua header
      'du_lieu' => $item,
      'trang_thai' => 'that_bai',
      'thong_bao' => $message,
      'loi' => $errors

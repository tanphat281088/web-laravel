...
class CreateQuanLyBanHangRequest extends FormRequest
{
  ...
  {
    return [
      // Thêm các quy tắc validation cho QuanLyBanHang ở đây
      'ma_don_hang' => 'required|string|max:255|unique:don_hangs,ma_don_hang',
      'ngay_tao_don_hang' => 'required|date',
      'loai_khach_hang' => 'required|integer|in:0,1',
      ...
      'giam_gia' => 'required|integer',
      'chi_phi' => 'required|integer',
      'loai_thanh_toan' => 'required|integer|in:0,1,2',
      'so_tien_da_thanh_toan' => 'nullable|integer',
      ...
      'danh_sach_san_pham.*.so_luong' => 'required|integer|min:1',

      // ✅ MỚI: Bắt buộc chọn 1 trong 2 loại giá
      'danh_sach_san_pham.*.loai_gia' => 'required|integer|in:1,2',
      // không bắt buộc don_gia vì Service sẽ thiết lập/ghi đè theo lô hoặc theo loại giá
    ];
  }
